...
<div class="container">
  ...
  <header>
    <nav style="" class="secondary_header" id="menu">
      <ul>
		<li><img onclick="location.href='Untitled-1.php?xy=<?php  echo $_GET['xy'] ?>'" src="https://cdn.discordapp.com/attachments/697470483813367821/777891384098291732/DGpalette2.png" ></li>
        <li class="d12"  ><a class="links hacked" href="Untitled-1.php?xy=<?php  echo $_GET['xy'] ?>" style="font-size:150%;">Home</a></li>
		...
		<div   class="filter" id="filter">
		 <form method="POST" action="">
			 <table class="try " style="width: 100%;">
				<tr>
					<td>
						<input type="checkbox"  name='gen[]' value="RPG">
						<label for="gen">RPG</label><br>
					</td>
					<td>
						<input type="checkbox"  name='gen[]' value="FPS">
						<label for="gen">FPS</label><br>
					</td>
				</tr>
				 <tr>
					<td>
						<input type="checkbox" name='gen[]' value="ADVENTURE">
						<label for="gen">Adventure</label><br>
					</td>
					 <td>
						<input type="checkbox" name='gen[]' value="SIMULATION">
						<label for="gen">Simulation</label><br>
					</td>
  ...
  <?php
	...
	if(!empty($_POST['gen'])) {
        ...
        foreach($_POST['gen'] as $value){
			...
			$res=mysqli_query($conn,"select * from products where Category='$value' ");
			if (!$res) {
				printf("Error: %s\n", mysqli_error($conn));
				exit();
			}
